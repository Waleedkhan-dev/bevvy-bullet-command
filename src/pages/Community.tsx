...
import { 
  Users, UserCheck, Crown, FlaskConical, MessageSquare, 
  Star, Quote, Check, Clock, X, Send, Trophy, Target,
  TrendingUp, Hash, Megaphone
} from "lucide-react";
...
const testimonials = [
  {
    id: 1,
    quote: "Bevvy Bullet has completely changed how I approach my morning routine. Can't wait for launch!",
    author: "Sarah M.",
    rating: 5,
    status: "approved",
    source: "Discord",
  ...
  {
    id: 3,
    quote: "Finally, a solution that actually works. The team behind this is amazing.",
    author: "Jessica L.",
    rating: 5,
    status: "pending",
    source: "Survey",
  ...
  {
    id: 4,
    quote: "I've tried everything and this is by far the best. Worth every penny.",
    author: "David K.",
    rating: 4,
    status: "approved",
    source: "Discord",
...
const recentActivity = [
  ...
  { type: "message", user: "VIPMember", time: "5m ago", channel: "#vip-lounge" },
  { type: "testimonial", user: "Sarah M.", time: "12m ago", channel: "Discord" },
  ...
  return (
    <AppLayout>
      ...
      >
        ...
        <p className="text-muted-foreground">Monitor and engage with your community members</p>
      </motion.div>
            ...
            <CardContent>
              <div className="flex items-center gap-4 mb-4 p-3 bg-muted/20 rounded-lg">
                <div className="text-center">
                  <p className="text-3xl font-mono font-bold text-primary">385</p>
                ...
                {channels.map((channel) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-foreground">{channel.name.replace('#', '')}</span>
                      {channel.active && (
                ...
                {recentActivity.map((activity, index) => (
                  ...
                  >
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                      activity.type === 'join' ? 'bg-accent/20' :
                      activity.type === 'message' ? 'bg-primary/20' :
                      activity.type === 'testimonial' ? 'bg-warning/20' :
                      'bg-secondary/20'
                    }`}>
                      {activity.type === 'join' && <UserCheck className="w-4 h-4 text-accent" />}
                      {activity.type === 'message' && <MessageSquare className="w-4 h-4 text-primary" />}
                      {activity.type === 'testimonial' && <Quote className="w-4 h-4 text-warning" />}
                      {activity.type === 'badge' && <Trophy className="w-4 h-4 text-secondary" />}
                    </div>
          ...
          <Card className="glass-card border-border/50">
            ...
            <CardContent>
              ...
              <div className="grid grid-cols-4 gap-2 mb-4 p-3 bg-muted/20 rounded-lg">
                <div className="text-center">
                  <p className="text-xl font-mono font-bold text-foreground">23</p>
          ...
          <Card className="glass-card border-border/50">
            ...
            <CardContent>
              ...
              <div className="mb-4">
                ...
                <p className="text-xs text-muted-foreground mt-1">12 spots remaining</p>
              </div>

              {/* Training Modules */}
              <div className="p-3 bg-muted/20 rounded-lg mb-4">
                <p className="text-sm font-medium text-foreground mb-2">Training Completion</p>
                <div className="grid grid-cols-4 gap-2">
                  {['Module 1', 'Module 2', 'Module 3', 'Module 4'].map((module, i) => (
                    <div key={module} className="text-center">
                      <div className={`w-10 h-10 mx-auto rounded-full flex items-center justify-center ${
                        i < 3 ? 'bg-accent/20 text-accent' : 'bg-muted/30 text-muted-foreground'
                      }`}>
                        {i < 3 ? <Check className="w-5 h-5" /> : <Clock className="w-5 h-5" />}
                      </div>
                      <p className="text-xs text-muted-foreground mt-1">M{i + 1}</p>
                    </div>
                  ))}
                </div>
              </div>
                ...
                {launchSquadMembers.map((member, index) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-muted-foreground">{member.modules}/4</span>
                    </div>
                  </div>
                ))}
              </div>
